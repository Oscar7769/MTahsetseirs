...
在傳統的有限差分時域法(FDTD)中，若直接將具有曲面或斜面的材料介面離散化到YEE晶格上，會產生階梯效應(Staircasing Effect)。
這種描述幾何結構的不連續性，使實際幾何邊界被近似為鋸齒狀結構，如圖~\ref{fig:Subpixel} 所示。這種幾何誤差會在材料介面的描述上降低其精確程度，
使原本具備二階空間精確度的FDTD退化為一階收斂，並造成共振頻率、散射特性及傳輸效率等模擬結果的誤差。

為了改善材料介面的離散化精確度，MEEP 採用了基於微擾理論(Perturbation Theory)的次像素平滑(Subpixel Smoothing)技術。
這個技術的核心概念是在邊界穿越的 Yee 網格內，以等效介電常數取代原本不連續的材料分布，使材料介面在次像素尺度上可以有更平滑的描述。
...
因此能在不增加網格解析度的情況下，就提高模擬結果的計算精確度。
(第一段後
Staircasing vs Subpixel Smoothing)
...
為了降低 Yee 晶格離散化造成的反射，PML 區域內的人工導電率必須由介面開始逐漸增加，而不是突然地改變。
MEEP 採用多項式導電率分布:
\begin{align}
...
適當增加 PML 厚度可有效降低邊界的反射，因此在一般光學模擬中，通常會設定約一個工作波長以上的 PML 厚度，可以獲得良好的吸收效果。
(第一段
...
有限差分時域法（FDTD）在時間軸上推進的本質，使其天然具備在單次模擬中透過寬頻短脈衝（Short-pulse Source）激發同時獲取大範圍頻譜資訊的能力 。
在矽光子元件的設計中，最核心的分析指標通常是特定運作頻率下的穿透率、反射率或模態轉換效率。為了在時域計算中精確提取這些頻域物理量，
MEEP 會在預先設定的監測表面 $S$ 上計算正歸化 Poynting 通量譜 $P(\omega)$ ：
\begin{align}
P(\omega) = \Re \iint_{S} \left[ \mathbf{E}{\omega}(\mathbf{r})^* \times \mathbf{H}{\omega}(\mathbf{r}) \right] \cdot d\mathbf{A}
\label{eq:poynting_flux}
\end{align}
其中，$\mathbf{E}_{\omega}(\mathbf{r})$ 與 $\mathbf{H}_{\omega}(\mathbf{r})$ 分別為特定頻率 $\omega$ 下的複數電場與磁場空間振幅 。  在傳統的 FDTD 實作中，若要計算該通量，必須在整場模擬的時間步進過程中，將監測面 $S$ 上所有晶格點在每個時間步長的時域訊號 $\mathbf{E}(\mathbf{r}, t)$ 與 $\mathbf{H}(\mathbf{r}, t)$ 完整記錄於記憶體中，模擬結束後再以快速傅立葉轉換（FFT）或離散傅立葉轉換（DFT）進行後處理 。然而，若模擬的時間總步數 $T$ 極大且頻率解析度要求極高，這種作法會帶來高達 $\Theta(N_S T)$ 的記憶體儲存負擔（其中 $N_S$ 為監測面上的網格點數）。  為了解決這個嚴重的記憶體耗費問題，MEEP 採用了即時動態步進累加的離散時間傅立葉轉換（Discrete-Time Fourier Transform, DTFT）策略 。在模擬推進的每一個時間步長 $n$ 中，演算法會直接在內核中執行對指定頻率 $\omega$ 的傅立葉積分累加 ：
\begin{align}
\mathbf{E}{\omega}(\mathbf{r}) &= \sum{n} e^{i \omega n \Delta t} \mathbf{E}(\mathbf{r}, n \Delta t) \Delta t 
\label{eq:dtft_e}
\end{align}
\begin{align}
\mathbf{H}{\omega}(\mathbf{r}) &= \sum{n} e^{i \omega n \Delta t} \mathbf{H}(\mathbf{r}, n \Delta t) \Delta t 
\label{eq:dtft_h}
\end{align}

這種作法將記憶體複雜度大幅降低至 $\Theta(N + N_S F)$，其中 $F$ 為目標計算的頻率點數（通常 $F \ll T$）。
當時間步進終止後，式 \eqref{eq:poynting_flux} 即可直接利用這些即時生成的複數場振幅進行空間積分。  
在此基礎之上，針對多模態波導（如 Mode-Division Multiplexing, MDM 系統）或包含特定幾何彎折（如 90度波導彎折）的元件，
單純的總通量積分已不足以描述各特徵導模的傳輸行為。MEEP 結合了上述 DFT 複數振幅提取與特徵模態擴展（Mode Expansion）演算法，
利用內建的頻域特徵模態求解器（如 MPB 核心），將提取出的總場 $\mathbf{E}_{\omega}$ 與 $\mathbf{H}_{\omega}$ 
投影至該波導截面的正交歸化特徵導模（如 $\text{TE}_0$ 模態）上。此外，針對具備極高諧振品質因子（High-$Q$ Factor，
如 $Q \sim 10^6$ 以上）的微腔結構，由於常規 DFT 受限於時間視窗的傅立葉不確定性原理（Spectral Leakage），
MEEP 進一步整合了濾波對角化技術（Filter Diagonalization Method, FDM）。該技術能將短時間序列的時域採樣訊號轉化為一個小型的矩陣特徵值問題，
僅需耗費傳統曲線擬合（Curve Fitting）十分之一的模擬時間，便能以極高精確度提取出極其尖銳的共振頻率 $\omega_0$ 與高品質因子 $Q$ 。
