...
如圖~\ref{fig:DEMUX_Structure}所示。
%\begin{figure}[H]
%\centering
%\includegraphics[width=1.0\textwidth]{img/DEMUX_Structure.png}
%\caption{Mode Division Demultiplexer 的輸入、輸出通道以及其幾何結構的設計區域。}
%\label{fig:DEMUX_Structure}
%\end{figure}

接著，光源設定方面，我是使用了MEEP中的EigenModeSource，利用GaussianSource高斯脈衝作為基礎光源
...
其中，輸入 TE$_0$ 模態時，應使其能量盡可能傳輸至指定輸出通道；輸入 TE$_1$ 模態時，則應使其能量傳輸至另一指定輸出通道。
同時，非目標輸出通道中的模態能量應盡可能降低，以抑制模態串擾(crosstalk)。
...
在設計目標是能夠將模態分波解多工器中單一通道輸入的TE$_0$和TE$_1$模態有效的將能量分離至兩個輸出通道，
並且在輸出的兩個通道中能夠偵測到我們想要的分離出去的TE$_0$模態，而目標模態的能量必須越大越好，模態串擾要越小越好。
