...
第二個部分是探討因數分解機結合退火式演算法在光學設計上的應用以及其限制(Table~\ref{tab:fmqa_sa})，
...
\cite{rendle2010_factorization}\textbf{西元2010年}Rendle提出「因數分解機(Factorization Machines, FM)」，其數學表達式可直接映射為 QUBO 模型的二次多項式，將高維組合問題轉換為 QUBO 形式，使直接利用退火硬體加速最佳化成為可能。
...
\cite{kitai2020_fmqa}\textbf{西元2020年}Kitai與Tamura等東京大學團隊結合因數分解機與 D-Wave 量子退火硬體，提出 FMQA 架構，於超穎材料設計中突破了傳統物理直覺的限制。
\cite{Wilson_2021}\textbf{西元2021年}Wilson等人提出結合二元變分自編碼器與因數分解機的機器學習框架，將連續設計空間映射至低維二元潛在空間，並轉換為QUBO問題，利用量子退火與模擬退火進行反向設計。
...
2021 & Wilson et al. proposed a machine learning framework that combines a Binary VAE and a Factorization Machine to transform continuous inverse design problems into QUBO formulations, which are optimized using quantum annealing and simulated annealing.\cite{Wilson_2021} \\[1.5em]
...
為提升二維光學結構的優化效率與幾何表示能力，並且改善離散像素化設計所造成的階梯效應與光學散射損耗，本研究探討低維度編碼、潛在空間降維以及空間頻率平滑化等結構表示方法，
並將其結合 FMSA 反向設計架構。以下整理相關技術之發展脈絡。\
