...
def evaluate_mdm_mode(binary_vector, grid_rows, grid_cols, mode_name):
    ...
    if mode_name == 'TE1':
        ...
        input_flux = sim.get_eigenmode_coefficients(norm_flux, [2]).alpha[0, 0, 0]
        input_power = np.abs(input_flux)**2 + 1e-12
        
        through_coeff = sim.get_eigenmode_coefficients(flux_through, [2]).alpha[0, 0, 0]
        T_through = np.abs(through_coeff)**2 / input_power
        
        cross_top_coeff = sim.get_eigenmode_coefficients(flux_cross_top, [2]).alpha[0, 0, 0]
        T_cross_top = np.abs(cross_top_coeff)**2 / input_power
        
        cross_bot_coeff = sim.get_eigenmode_coefficients(flux_cross_bot, [2]).alpha[0, 0, 1]
        ...
        through_coeff = sim.get_eigenmode_coefficients(flux_through, [1], eig_parity=mp.EVEN_Y).alpha[0, 0, 0]
        T_through = np.abs(through_coeff)**2 / input_power
        
        cross_top_coeff = sim.get_eigenmode_coefficients(flux_cross_top, [1]).alpha[0, 0, 0]
        T_cross_top = np.abs(cross_top_coeff)**2 / input_power
        
        cross_bot_coeff = sim.get_eigenmode_coefficients(flux_cross_bot, [1]).alpha[0, 0, 1]
        T_cross_bot = np.abs(cross_bot_coeff)**2 / input_power
...
def perform_detailed_final_analysis(best_config, grid_rows, grid_cols, output_folder):
    ...
    for mode_name in mode_definitions:
        ...
        if mode_name == 'TE1':
            ...
            res_input = sim.get_eigenmode_coefficients(norm_flux, [2]).alpha[0, 0, 0]
            ...
            res_input = sim.get_eigenmode_coefficients(norm_flux, [1], eig_parity=mp.EVEN_Y).alpha[0, 0, 0]
            input_power = np.abs(res_input)**2 + 1e-12
...
def plot_fom_history(fom_history, output_dir):
    ...
    plt.ylabel("Best FOM Score", fontsize=14)
    plt.grid(True)
...
def parallel_evaluate(tasks, fom_cache, grid_rows, grid_cols):
    ...
    while jobs_done < num_to_run:
        ...
        if 'TE0' in partial_results[task_idx] and 'TE1' in partial_results[task_idx]:
            ...
            fom = (err_T0 + err_T1 + err_C0 + err_C1) / 4.0
