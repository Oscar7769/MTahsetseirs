...
def evaluate_mdm_mode(binary_vector, grid_rows, grid_cols, mode_name):
    ...
    monitor_size_x = mp.Vector3(WG_WIDTH * 2, 0) # Y方向波導的截面 
    
    mode_props = {
        'TE0': {'band_num': 1, 'parity': mp.EVEN_Y, 'global_band': 1},
        'TE1': {'band_num': 2, 'parity': mp.ODD_Y, 'global_band': 2},
        'TE2': {'band_num': 3, 'parity': mp.EVEN_Y, 'global_band': 3},
        'TE3': {'band_num': 4, 'parity': mp.ODD_Y, 'global_band': 4},
    }
    ...
    norm_flux = sim.add_mode_monitor(FCEN, 0, 1, in_flux_region)
    
    through_flux_region = mp.ModeRegion(center=mp.Vector3(mon_x_through, 0), size=monitor_size_y)
    flux_through = sim.add_mode_monitor(FCEN, 0, 1, through_flux_region)
    
    cross_top_region = mp.ModeRegion(center=mp.Vector3(0, mon_y_cross_top - 0.5), size=monitor_size_x)
    flux_cross_top = sim.add_mode_monitor(FCEN, 0, 1, cross_top_region)
    
    cross_bot_region = mp.ModeRegion(center=mp.Vector3(0, mon_y_cross_bot - 0.5), size=monitor_size_x)
    flux_cross_bot = sim.add_mode_monitor(FCEN, 0, 1, cross_bot_region)

    sim.run(until_after_sources=mp.stop_when_fields_decayed(50, mp.Ez, mp.Vector3(mon_x_through, WG_WIDTH/4), 1e-4))
    
    input_flux = sim.get_eigenmode_coefficients(norm_flux, [props['band_num']], eig_parity=props['parity']).alpha[0, 0, 0]
    input_power = np.abs(input_flux)**2 + 1e-12
    
    through_coeff = sim.get_eigenmode_coefficients(flux_through, [props['band_num']], eig_parity=props['parity']).alpha[0, 0, 0]
    T_through = np.abs(through_coeff)**2 / input_power
    
    # 對於正交波導 (Cross)，Meep 沒有 EVEN_X/ODD_X，因此必須不指定奇偶性，直接提取 global_band
    gb = props['global_band']
    
    cross_top_coeff = sim.get_eigenmode_coefficients(flux_cross_top, [gb]).alpha[0, 0, 0]
    T_cross_top = np.abs(cross_top_coeff)**2 / input_power
    
    cross_bot_coeff = sim.get_eigenmode_coefficients(flux_cross_bot, [gb]).alpha[0, 0, 1]
    T_cross_bot = np.abs(cross_bot_coeff)**2 / input_power

    sim.reset_meep()
    return {'through': T_through, 'cross_top': T_cross_top, 'cross_bot': T_cross_bot}
...
def perform_detailed_final_analysis(best_config, grid_rows, grid_cols, output_folder):
    ...
    mon_y_cross_top = MDM_LC / 2 + WG_LENGTH / 2
    mon_y_cross_bot = -MDM_LC / 2 - WG_LENGTH / 2
    monitor_size_y = mp.Vector3(0, WG_WIDTH * 2) # X方向波導的截面
    monitor_size_x = mp.Vector3(WG_WIDTH * 2, 0) # Y方向波導的截面 
    
    mode_definitions = ['TE0', 'TE1', 'TE2', 'TE3']
    detailed_results = {}
    ...
    for mode_name in mode_definitions:
        ...
        trans = T_through
        loss = T_cross_bot + T_cross_top
        trans_db = 10 * np.log10(trans + 1e-9)
        T_cross_top_db = 10 * np.log10(T_cross_top + 1e-9)
        T_cross_bot_db = 10 * np.log10(T_cross_bot + 1e-9)
        loss_db = 10 * np.log10(loss + 1e-9)
    
        print(f"  [Result] {mode_name} L->R (Through): {trans:.4f} ({trans_db:.2f} dB)")
        print(f"  [Result] {mode_name} L->T (Cross Top): {T_cross_top:.4f} ({T_cross_top_db:.2f} dB)")
        print(f"  [Result] {mode_name} L->B (Cross Bot): {T_cross_bot:.4f} ({T_cross_bot_db:.2f} dB)")
        print(f"  [Result] {mode_name} Loss/Scatter: {loss:.4f} ({loss_db:.2f} dB)")

        detailed_results[mode_name] = {
            "L->R": [float(trans), float(trans_db)],
            "L->T": [float(T_cross_top), float(T_cross_top_db)],
            "L->B": [float(T_cross_bot), float(T_cross_bot_db)],
            "Loss/Scatter": [float(loss), float(loss_db)]
        }
        ...
        fig.colorbar(im, cax=cax, label='Intensity |Ez|^2')
...
def plot_optimization_trajectory(foms, split_index, output_folder):
    ...
    plt.ylabel('Optimization FOM (Lower is Better)', fontsize=14)
...
def calculate_fom_from_partial(trans_res):
    TE0_T_through = trans_res['TE0']['through']
    TE0_T_cross_top = trans_res['TE0']['cross_top']
    TE0_T_cross_bot = trans_res['TE0']['cross_bot']
    
    TE1_T_through = trans_res['TE1']['through']
    TE1_T_cross_top = trans_res['TE1']['cross_top']
    TE1_T_cross_bot = trans_res['TE1']['cross_bot']
    
    TE2_T_through = trans_res['TE2']['through']
    TE2_T_cross_top = trans_res['TE2']['cross_top']
    TE2_T_cross_bot = trans_res['TE2']['cross_bot']
    
    TE3_T_through = trans_res['TE3']['through']
    TE3_T_cross_top = trans_res['TE3']['cross_top']
    TE3_T_cross_bot = trans_res['TE3']['cross_bot']
    
    TE0_crosstalk = TE0_T_cross_top + TE0_T_cross_bot
    TE1_crosstalk = TE1_T_cross_top + TE1_T_cross_bot
    TE2_crosstalk = TE2_T_cross_top + TE2_T_cross_bot
    TE3_crosstalk = TE3_T_cross_top + TE3_T_cross_bot

    err_T0 = (1.0 - TE0_T_through)**2
    err_T1 = (1.0 - TE1_T_through)**2
    err_T2 = (1.0 - TE2_T_through)**2
    err_T3 = (1.0 - TE3_T_through)**2
    
    err_C0 = (TE0_crosstalk)**2
    err_C1 = (TE1_crosstalk)**2
    err_C2 = (TE2_crosstalk)**2
    err_C3 = (TE3_crosstalk)**2

    return (err_T0 + err_T1 + err_T2 + err_T3 + err_C0 + err_C1 + err_C2 + err_C3) / 8.0
...
def parallel_evaluate(tasks, fom_cache, grid_rows, grid_cols):
    ...
    for i, config in enumerate(tasks):
        ...
        if config_tuple in fom_cache: 
            ...
            tasks_to_run.append((i, config, 'TE2'))
            tasks_to_run.append((i, config, 'TE3'))
            partial_results[i] = {}

    if not tasks_to_run: return results

    if size == 1:
        # 單核運行：Master 自行迴圈執行所有子任務並組裝結果
        for task_idx, config, mode_name in tasks_to_run:
            res_dict = evaluate_mdm_mode(config, grid_rows, grid_cols, mode_name)
            partial_results[task_idx][mode_name] = res_dict
            if 'TE0' in partial_results[task_idx] and 'TE1' in partial_results[task_idx] and 'TE2' in partial_results[task_idx] and 'TE3' in partial_results[task_idx]:
                fom = calculate_fom_from_partial(partial_results[task_idx])
                results[task_idx] = fom
                fom_cache[tuple(tasks[task_idx])] = fom
        return results

    # 多核非同步派發
    num_subtasks = len(tasks_to_run)
    sent_subjobs = 0
    subjobs_done = 0
    
    for worker_rank in range(1, min(size, num_subtasks + 1)):
        comm.send(tasks_to_run[sent_subjobs], dest=worker_rank, tag=WORK_TAG)
        sent_subjobs += 1

    while subjobs_done < num_subtasks:
        status = MPI.Status()
        ...
        if sent_subjobs < num_subtasks:
            comm.send(tasks_to_run[sent_subjobs], dest=status.Get_source(), tag=WORK_TAG)
            sent_subjobs += 1
...
def master_node():
    ...
    parser.add_argument('--name', type=str, default=f'4mode_Crossing_waveguide_{PARAMS["INIT_DATASET_SIZE"]}_{PARAMS["ITERATIONS"]}x{PARAMS["ADDING_NUM"]}', help='Job Name')
    ...
    experiment_log = {
        ...
        "results": {
            ...
            "TE0 L -> R": f"{final_res['TE0']['L->R'][0]:.4f} ({final_res['TE0']['L->R'][1]:.2f} dB)",
            "TE0 L -> T": f"{final_res['TE0']['L->T'][0]:.4f} ({final_res['TE0']['L->T'][1]:.2f} dB)",
            "TE0 L -> B": f"{final_res['TE0']['L->B'][0]:.4f} ({final_res['TE0']['L->B'][1]:.2f} dB)",
            "TE0 Loss/Scatter": f"{final_res['TE0']['Loss/Scatter'][0]:.4f} ({final_res['TE0']['Loss/Scatter'][1]:.2f} dB)",
            "TE1 L -> R": f"{final_res['TE1']['L->R'][0]:.4f} ({final_res['TE1']['L->R'][1]:.2f} dB)",
            "TE1 L -> T": f"{final_res['TE1']['L->T'][0]:.4f} ({final_res['TE1']['L->T'][1]:.2f} dB)",
            "TE1 L -> B": f"{final_res['TE1']['L->B'][0]:.4f} ({final_res['TE1']['L->B'][1]:.2f} dB)",
            "TE1 Loss/Scatter": f"{final_res['TE1']['Loss/Scatter'][0]:.4f} ({final_res['TE1']['Loss/Scatter'][1]:.2f} dB)",
            "TE2 L -> R": f"{final_res['TE2']['L->R'][0]:.4f} ({final_res['TE2']['L->R'][1]:.2f} dB)",
            "TE2 L -> T": f"{final_res['TE2']['L->T'][0]:.4f} ({final_res['TE2']['L->T'][1]:.2f} dB)",
            "TE2 L -> B": f"{final_res['TE2']['L->B'][0]:.4f} ({final_res['TE2']['L->B'][1]:.2f} dB)",
            "TE2 Loss/Scatter": f"{final_res['TE2']['Loss/Scatter'][0]:.4f} ({final_res['TE2']['Loss/Scatter'][1]:.2f} dB)",
            "TE3 L -> R": f"{final_res['TE3']['L->R'][0]:.4f} ({final_res['TE3']['L->R'][1]:.2f} dB)",
            "TE3 L -> T": f"{final_res['TE3']['L->T'][0]:.4f} ({final_res['TE3']['L->T'][1]:.2f} dB)",
            "TE3 L -> B": f"{final_res['TE3']['L->B'][0]:.4f} ({final_res['TE3']['L->B'][1]:.2f} dB)",
            "TE3 Loss/Scatter": f"{final_res['TE3']['Loss/Scatter'][0]:.4f} ({final_res['TE3']['Loss/Scatter'][1]:.2f} dB)"
        }
    }
