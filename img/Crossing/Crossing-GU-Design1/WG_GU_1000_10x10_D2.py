...
def evaluate_mdm_fom(binary_vector, grid_rows, grid_cols):
    ...
    # 計算南北兩側漏光 (不加 parity 讓它捕捉所有跑到該通道的 TE0)
...
def perform_detailed_final_analysis(best_config, grid_rows, grid_cols, output_folder):
    ...
    print(f"  [Result] L->R (Through): {trans:.4f} ({trans_db:.2f} dB)")
    print(f"  [Result] L->T (Cross Top): {T_cross_top:.4f}")
    print(f"  [Result] L->B (Cross Bot): {T_cross_bot:.4f}")
    ...
    return {
        ...
        "Cross_Top": float(T_cross_top),
        "Cross_Bot": float(T_cross_bot),
        "Loss": float(loss)
    }
...
def master_node():
    ...
    experiment_log = {
        ...
        "results": {
            ...
            "all_evaluated_foms": foms.tolist(), # [修正 4] JSON Log 中也移除 np.abs
            "final_analysis": final_res,
            "Transmission": final_res["Transmission"],
            "Transmission_dB": final_res["Transmission_dB"],
            "Cross_Top": final_res["Cross_Top"],
            "Cross_Bot": final_res["Cross_Bot"],
            "Loss": final_res["Loss"]
        }
    }
